...
private int g;
	...
	//private static final String IMGURL_REG1 = "src=[^>]*[^/].(?:jpg|jpeg|bmp|gif)(?:\\\"|\\'|\\s)";
	//private static final String IMGURL_REG2= "<img.*class=(.*?)[^>]*?>";
	private static final String IMGURL_REG3 ="(?x)(src|SRC|background|BACKGROUND)=('|\")/?(([\\w-]+/)*([\\w-]+\\.(jpg|JPG|png|PNG|gif|GIF)))('|\")";
	private static final String IMGURL_REG4 ="(?x)(src|SRC|background|BACKGROUND)=('|\")(http://([\\w-]+\\.)+[\\w-]+(:[0-9]+)*(/[\\w-]+)*(/[\\w-]+\\.(jpg|JPG|png|PNG|gif|GIF)))('|\")";
	// 获取src路径的正则
...
//	  public int countPic(List<String> listimageurl ) throws IOException { 
//		  List<String> listImageSrc = new ArrayList<String>();
//	        Pattern pattern = Pattern.compile("src=[^>]*[^/].(?:jpg|jpeg|bmp|gif)(?:\\\"|\\'|\\s)", Pattern.CASE_INSENSITIVE);
...
//	    }
	...
	public List<String> getImageUrl(String content) {     //找到圖片的url
		...
		Matcher matcher3 = Pattern.compile(IMGURL_REG3).matcher(content);
		Matcher matcher4 = Pattern.compile(IMGURL_REG4).matcher(content);
		List<String> listimgurl = new ArrayList<String>();

		while (matcher3.find()) {
			listimgurl.add(matcher3.group());
		}
		while (matcher4.find()) {
			listimgurl.add(matcher4.group());
		}
		return listimgurl;
	}
